...
async function getRunningEmacsPids(): Promise<number[]> {
  try {
    const { stdout } = await execAsync(
      'ps -eo pid,comm | grep -v grep | grep -v emacsclient | grep -iE "\\bemacs\\b" | awk "{print $1}"'
    );
    const pids = stdout
      .trim()
      .split("\n")
...
// Notify a specific Emacs instance, or find the single running one
...
// Notify Emacs about the port — uses direct instance targeting
async function notifyEmacsPort(port: number, projectRoot: string, targetInstanceId?: number): Promise<void> {
  const success = await notifyEmacsInstance(port, projectRoot, targetInstanceId);
  if (!success) {
    log(`Failed to notify any Emacs instance about port ${port} for project ${projectRoot}`);
  }
}
...
async function main() {
  ...
  // Notify Emacs about the assigned port using direct instance targeting
  await notifyEmacsPort(port, sessionId, targetInstanceId);
...
async function cleanup() {
  ...
  if (targetInstanceId) {
    ...
    try {
      await execAsync(`emacsclient -s ${serverName} --eval '${elisp}'`);
      log(`Unregistered port for project ${projectRoot} from instance ${targetInstanceId}`);
    } catch (error) {
      log(`Failed to unregister port from instance ${targetInstanceId}: ${error}`);
    }
  } else {
    log(`No target instance found for cleanup of project ${projectRoot}`);
  }
