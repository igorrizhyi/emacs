    ...
    (when (and (>= (length parsed) 3)
      ...
      (dolist (row parsed)
        (when (my/agent-shell--separator-row-p row)
          (setq has-sep t))
        (setq ncols (max (or ncols 0) (length row))))
      ;; Pad short rows with empty cells to match ncols
      (setq parsed (mapcar (lambda (row)
                             (let ((deficit (- ncols (length row))))
                               (if (> deficit 0)
                                   (append row (make-list deficit ""))
                                 row)))
                           parsed))
