...
(defun monet--get-tool-handler (name)
  ...
  (let ((handler
         (pcase name
           ...
           ("terminalExecute" #'monet--tool-terminal-execute-handler)
           (_ nil))))
    handler))
...
(defun monet--tool-terminal-execute-handler (params _session)
  "MCP handler for terminalExecute tool.
PARAMS should contain command and optional timeout.
_SESSION is unused."
  (let ((command (alist-get 'command params))
        (timeout (or (alist-get 'timeout params) 30)))
    (if (and (featurep 'claude-code) (fboundp 'claude-code--execute-ai-command))
        (let ((result (claude-code--execute-ai-command command)))
      ...
      `((isError . t)
        (content . (((type . "text")
                    (text . "Terminal execution not available: claude-code.el not loaded or AI terminal not connected"))))))))
  ...
  (let ((tools
         (list
          ...
          `((name . "terminalExecute")
            (description . "Execute a command in the connected Emacs terminal")
            (inputSchema . ((type . "object")
...
(defun monet--get-selection ()
  ...
  (when (monet--is-valid-buffer-for-selection (current-buffer))
    (let* ((file-path (or buffer-file-name 
                         (concat "terminal://" (buffer-name (current-buffer)))))
           (point-pos (point))
...
(defun monet--track-selection-change ()
  ...
  (when (and (monet--is-valid-buffer-for-selection (current-buffer))
