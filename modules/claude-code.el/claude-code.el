...
(defvar claude-code-command-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "T") 'claude-code--toggle-terminal-ai-connection)
    (define-key map (kbd "A") 'claude-code--connect-terminal-to-ai)
    (define-key map (kbd "D") 'claude-code--disconnect-terminal-from-ai)
    map)
...
(transient-define-prefix claude-code-transient ()
  ...
  ["Claude Commands"
   ...
   ["AI Terminal"
    ...
    ("A" "Connect terminal to AI" claude-code--connect-terminal-to-ai)
...
(defun claude-code-send-text ()
  "Interactively prompt for text and send it to Claude followed by <return>.

This function reads text from the user, sends it directly to Claude,
and then sends a return key to execute the command."
  (interactive)
...
(defun claude-code-send-emacs-terminal ()
  "Interactively prompt for input and send it to Claude via /emacs-terminal command.

This function reads input from the user and sends it to Claude using the
/emacs-terminal command followed by the user's input."
  (interactive)
  (let ((input (read-string "Emacs terminal input: ")))
    (claude-code--do-send-command (format "/emacs-terminal %s" input))))
...
;;;;; Core AI Command Execution Functions
...
Returns a plist with :success, :output, and :error-message."
  (if-let ((terminal-buffer claude-code--ai-connected-terminal))
      (if (buffer-live-p terminal-buffer)
          (with-current-buffer terminal-buffer
            (let ((start-marker (point-marker))
                  (start-time (current-time)))
              ;; Validate command first
              (if (claude-code--validate-ai-command command)
                  (progn
                    ;; Inject command into terminal
                    (claude-code--send-to-any-terminal command)
                    (claude-code--send-to-any-terminal (kbd "RET"))
                    
                    ;; Wait for command completion and capture output
                    (claude-code--wait-for-command-completion start-marker start-time))
                (list :success nil :error-message "Command rejected by validation"))))
        (list :success nil :error-message "AI-connected terminal buffer is no longer live"))
...
START-TIME is when the command was initiated."
  (let ((timeout-time (time-add start-time claude-code-ai-terminal-timeout))
        (completion-marker nil)
        (output ""))
    
    ;; Poll for completion
    (while (and (not completion-marker)
                (time-less-p (current-time) timeout-time))
      (sit-for 0.1)  ; Brief pause between checks
      (setq completion-marker (claude-code--detect-command-completion start-marker)))
    
    (if completion-marker
        (progn
          (setq output (claude-code--extract-command-output start-marker completion-marker))
          (list :success t :output output))
      (list :success nil :error-message "Command execution timed out"))))

(defun claude-code--detect-command-completion (start-marker)
  "Detect when command has completed since START-MARKER.
Returns marker position of completion or nil if still running."
  (save-excursion
    (goto-char start-marker)
    (let ((search-end (point-max)))
      ;; Look for common prompt patterns that indicate command completion
      ;; This is a simple heuristic - can be improved based on shell configuration
      (when (re-search-forward "\\(\\$\\|#\\|>\\)\\s-*$" search-end t)
        (point-marker)))))

(defun claude-code--extract-command-output (start-marker end-marker)
  "Extract and clean command output between START-MARKER and END-MARKER."
...
(defun claude-code--send-to-any-terminal (string)
  "Send STRING to the current terminal buffer, regardless of terminal type."
  (cond
   ;; Claude terminal buffers - use existing backend system
   ((claude-code--buffer-p (current-buffer))
    (claude-code--term-send-string claude-code-terminal-backend string))
   
   ;; vterm buffers
   ((and (featurep 'vterm) (eq major-mode 'vterm-mode))
    (if (equal string (kbd "RET"))
        (vterm-send-key "\C-m")
      (vterm-send-string string)))
   
   ;; eat buffers
   ((and (featurep 'eat) (eq major-mode 'eat-mode))
    (if (equal string (kbd "RET"))
        (eat-term-send-string eat-terminal "\C-m")
      (eat-term-send-string eat-terminal string)))
   
   ;; Built-in term buffers
...
(defun claude-code--is-terminal-buffer-p (buffer)
  "Check if BUFFER is a terminal buffer (any type: vterm, eat, term, etc.)."
  (with-current-buffer buffer
    (and 
     ;; Exclude Claude Code buffers
     (not (claude-code--buffer-p buffer))
     ;; Check for terminal buffer types
     (or 
      ;; vterm buffers
      (and (featurep 'vterm) (eq major-mode 'vterm-mode))
      ;; eat buffers  
...
(defun claude-code--connect-terminal-to-ai (&optional buffer)
  ...
  (let ((target-buffer (or buffer (current-buffer))))
    (unless (claude-code--is-terminal-buffer-p target-buffer)
      (error "Buffer is not a terminal buffer"))
