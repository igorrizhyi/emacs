...
class TestIsRetryableError:
    ...
    def test_matches_500(self):
        assert is_retryable_error("Error 500: Internal Server Error") is True
    ...
    def test_matches_temporarily_unavailable(self):
        assert is_retryable_error("Model temporarily unavailable") is True
...
class TestRetryWithBackoff:
    ...
    async def test_succeeds_first_try(self, mock_sleep: AsyncMock):
        ...
        result = await retry_with_backoff(fn, backoff_seconds=[2.0, 5.0])
        assert result == "ok"
    ...
    async def test_retries_on_capacity_error(self, mock_sleep: AsyncMock):
        ...
        result = await retry_with_backoff(fn, backoff_seconds=[0.1, 0.1])
    ...
    async def test_raises_non_retryable(self, mock_sleep: AsyncMock):
        ...
        with pytest.raises(RuntimeError, match="Invalid API key"):
            await retry_with_backoff(fn, backoff_seconds=[2.0, 5.0])
        fn.assert_awaited_once()
        mock_sleep.assert_not_awaited()
    ...
    async def test_exhausts_retries(self, mock_sleep: AsyncMock):
        ...
        with pytest.raises(RuntimeError, match="No capacity available"):
            await retry_with_backoff(fn, backoff_seconds=[0.1, 0.1])
